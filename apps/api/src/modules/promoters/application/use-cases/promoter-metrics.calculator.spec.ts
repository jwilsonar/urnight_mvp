...
function invitationList(
  count = 1,
): PromoterAnalyticsFacts["invitationLists"][number] {
  return {
    ...facts().assignments[0]!,
    allocationId: "allocation-1",
    allocatedStock: count,
    issuedCodes: Array.from({ length: count }, (_, index) => ({
      id: `code-${index}`,
      code: `ANA-${index}`,
    })),
  };
...
describe("calculatePromoterMetrics", () => {
  it("separa lista emitida, canjes y asistencia: 40 invitaciones, 10 canjes y 8 ingresos", () => {
    const issuedCodes = Array.from({ length: 40 }, (_, index) => ({
      id: `code-${index}`,
      code: `ANA-${index}`,
    }));
    const attributions = Array.from({ length: 10 }, (_, index) =>
      attribution({
        orderId: `order-${index}`,
        code: `ANA-${index}`,
      }),
    ...
    const tickets = Array.from({ length: 10 }, (_, index) =>
      ticket({
        ...
        status: index < 8 ? "used" : "valid",
        usedAt:
          index < 8 ? new Date(`2026-08-01T0${index + 1}:30:00.000Z`) : null,
      }),
    );
    const input = {
      ...facts({ attributions, tickets }),
      invitationLists: [
        {
          ...facts().assignments[0]!,
          allocationId: "allocation-1",
    ...
    } as unknown as PromoterAnalyticsFacts;
    ...
    expect(result.totals.invitedCount).toBe(40);
    expect(result.totals.redeemedCount).toBe(10);
    expect(result.totals.attendedCount).toBe(8);
    expect(result.totals.redemptionRate).toBe(25);
    expect(result.totals.attendanceRate).toBe(20);
    expect(result.totals.salesCount).toBe(10);
  });
  ...
  it("devuelve ceros, no NaN, cuando la lista está vacía", () => {
    ...
    expect(result.totals.invitedCount).toBe(0);
    expect(result.totals.redeemedCount).toBe(0);
  ...
  it("cuenta en lista a quien tiene ticket vigente, pero no como asistencia si nunca entró", () => {
    const result = calculatePromoterMetrics(
      { id: PROMOTER, name: "Ana", companyId: "company-1" },
      facts({
        invitationLists: [invitationList()],
        attributions: [attribution()],
        tickets: [ticket()],
      }),
    );

    expect(result.totals.invitedCount).toBe(1);
    expect(result.totals.redeemedCount).toBe(1);
    expect(result.totals.attendedCount).toBe(0);
    expect(result.totals.attendanceRate).toBe(0);
  });
  ...
  it("muestra el evento cancelado, pero excluye sus números de los totales", () => {
    ...
    expect(result.events[0]?.invitedCount).toBe(0);
    expect(result.totals.invitedCount).toBe(0);
    ...
    expect(result.conflicts).toEqual([
      {
        ...
        amount: 100,
        currency: "PEN",
        promoterIds: [PROMOTER, "66666666-6666-6666-6666-666666666666"],
      },
    ]);
  });

  it("no inventa una comisión cuando la venta no tiene snapshot persistido", () => {
    const result = calculatePromoterMetrics(
      { id: PROMOTER, name: "Ana", companyId: "company-1" },
      facts({ attributions: [attribution()], tickets: [ticket()] }),
    );

    expect(result.events[0]?.sales[0]?.commissionAmount).toBeNull();
    expect(result.totals.commissionPendingCount).toBe(1);
  });
});
