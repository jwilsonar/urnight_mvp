...
import { EventNotFoundError, EventNotPublishableError } from '../../domain/errors/events.errors';
import { EventPublishedEvent } from '../../domain/events/events.events';
...
/** Caso de uso: publicar evento (admin_local del local dueño). */
@Injectable()
export class PublishEventUseCase {
  private readonly log = createLogger(PublishEventUseCase.name);
  ...
  async execute(input: { eventId: string; scope: TenantScope }): Promise<Event> {
    this.log.debug({ eventId: input.eventId }, 'events.event.publish.started');
    ...
    if (!event.canPublish()) {
      this.log.warn({ eventId: input.eventId }, 'events.event.not_ready');
      throw new EventNotPublishableError();
    }
    event.publish();
    const saved = await this.events.update(event);
    await this.bus.publish(new EventPublishedEvent({ eventId: saved.id, localId: saved.localId }));
    this.log.info({ eventId: saved.id }, 'events.event.published');
    return saved;
  }
}
