import { Badge, Card, CardContent, CardHeader, CardTitle, cn } from '@urnight/ui';
...
const NIVEL_LABEL: Record<CrowdNivelDemo, string> = {
  bajo: 'Ambiente tranquilo',
  medio: 'Buen ambiente',
  lleno: 'Casi lleno',
};
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between gap-3 space-y-0 pb-3">
        <CardTitle className="font-heading text-lg">Ambiente ahora</CardTitle>
      ...
      <CardContent>
        ...
        <p className="mt-4 text-sm font-semibold">
          {crowd.esperaMin > 0 ? `Espera aprox. ${crowd.esperaMin} min` : 'Sin cola'}
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
          Actualizado {crowd.actualizadoLabel}
        </p>
      </CardContent>
    </Card>
  );
